...
def get_nigeria_master_data(pbf_path):
    try:
        ...
        master_df = (
            master_df.dropna(subset=["addr:street", "name"], how="all")
            .fillna("")
            .head(5000)
        )
...
def main():
    ...
    if not pbf_filename or not os.path.exists(pbf_filename):
        ...
        OUTPUT_DIR = "./nigeria-address-ner"

    # Step A: Prepare Dataset
    logger.error(f"PBF file '{pbf_filename}' not found on disk.")
...
if __name__ == "__main__":
    ...
    run_live_test("./nigeria-address-ner")
