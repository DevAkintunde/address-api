...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=f"""
Examples:
  python sazzy.py                                    # Train on Lagos (default)
  python sazzy.py --region all                       # Train on all Nigeria
  python sazzy.py --samples 50000                    # Train on 50k Lagos buildings
  python sazzy.py --region lagos,abuja,kano          # Train on specific states
  python sazzy.py --data ./custom/path.geojsonl      # Use custom data file
    ...
    if checkpoint_files:
        ...
        for cf in sorted(checkpoint_files):
            ...
            if len(parts) >= 2:
                region = parts[0]
                samples = parts[1]
                logger.info(f"  - {cf} ({size_mb:.1f} MB) [Region: {region}, Samples: {samples}]")
            else:
                logger.info(f"  - {cf} ({size_mb:.1f} MB)")
    else:
        logger.info("No building checkpoints found in output directory")

def get_checkpoint_path(output_dir, region, samples):
...
def load_microsoft_buildings(file_path, sample_size=100000, region_filter="lagos"):
    ...
    logger.info(f"Final sample: {len(buildings):,} buildings")
    return buildings
...
def main():
    ...
    checkpoint_file = get_checkpoint_path(args.output, args.region, args.samples)
    ...
    if os.path.exists(checkpoint_file) and not args.force_reload:
        ...
        logger.info("This will take 10-15 minutes for 100,000 buildings...")
    ...
    for idx, (_, building) in enumerate(buildings.iterrows()):
        ...
        if (idx + 1) % 10000 == 0:
            logger.info(f"  Generated {idx + 1:,} addresses...")
